...
class CollectionDashboard extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      currentPage: 1,
      postsPerPage: 6, //<--- TODO change this after adding more items
      loading: false,
    };
  }
    ...
    this.setState((state) => {
      if (state.sort !== "") {
        this.props.products.sort((a, b) =>
          state.sort === "lowest" ? (a.basePrice > b.basePrice ? 1 : -1) : {}
        );
        this.props.products.sort((a, b) =>
          state.sort === "highest" ? (a.basePrice < b.basePrice ? 1 : -1) : {}
        );
        this.props.products.sort((a, b) =>
          state.sort === "atoz"
        ...
        this.props.products.sort((a, b) =>
          a.rating && b.rating && a.rating.totalRating < b.rating.totalRating
            ? 1
            : -1
        );
      }
  ...
  UNSAFE_componentWillReceiveProps() {
    this.setState({ postsPerPage: 6 }); //<--- TODO change this after adding more items
  }
  render() {
    const {
      store,
    ...
    const seeMore = () => {
      this.setState({ postsPerPage: postsPerPage + 6 }); //<--- TODO change this after adding more items
    };
    ...
    if (!isLoaded(products) || isEmpty(products) || !availability)
    ...
    return (
      <div>
        {category && (
          <Helmet>
            <title>
              {_.startCase(category)} | {store.storeName}
            </title>
          </Helmet>
        )}
        <Grid  className='collection-main-grid'  >
          ...
          <Grid.Row className='collection-row-see-more'>
            {/*<Pagination
              totalPages={3}
              activePage={this.state._page}
              onPageChange={this.onChangePage}
            />*/}
            <a className="see-more" onClick={seeMore}>
              SEE MORE
            </a>
          </Grid.Row>
          <Grid.Row></Grid.Row>
